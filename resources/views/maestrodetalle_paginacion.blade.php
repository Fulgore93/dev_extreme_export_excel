    ...
    $(document).ready(async function(e) {
        ...
        var items = new DevExpress.data.CustomStore({ // función para el origen de datos
            ...
            load: function(loadOptions) {
                ...
                var params = {
                    skip: loadOptions.skip || 0, // A partir de qué registro se cargarán los datos
                    take: loadOptions.take || 10, // Cuántos registros se cargarán
                    page: Math.floor(loadOptions.skip / (loadOptions.take || 10)) + 1
                };
                return sendRequest(url_list, "GET", params).then(function(response) {
                    return {
                        data: response.data,         // Los datos de la página actual
                        totalCount: response.totalCount // El total de registros
                    };
                });
            }
        });  
                            ...
                            DevExpress.excelExporter.exportDataGrid({
                                ...
                                for(var i = 0; i < masterRows.length; i++) {
                                    ...
                                    rowDatagrid.detalle.forEach((task, index) => {
                                        ...
                                        columns.forEach((columnName, currentColumnIndex) => {
                                            Object.assign(row.getCell(columnIndex + currentColumnIndex), {
                                                value: task[columnName], // ojo con los nombres de las columnas que deben ser iguales entre el dato y lo que se definió en columnas
                                                fill: { type: 'pattern', pattern:'solid', fgColor: { argb: 'ffffff' } },
                                                border: { bottom: borderStyle, left: borderStyle, right: borderStyle,top: borderStyle }
                                            });
                                        });
